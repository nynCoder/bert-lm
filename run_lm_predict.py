...
def parse_result(result, all_tokens, output_file=None):
  with tf.gfile.GFile(output_file, "w") as writer:
    ...
    for word_loss in result:
      ...
      # end of a sentence
      if all_tokens[i] == "[SEP]":
        sentence["tokens"] = tokens#单词的概率值
        sentence["ppl"] = float(np.exp(sentence_loss / word_count_per_sent))#句子的困惑度
    ...
    if output_file is not None:
      ...
      for sen in sentences:
          print("".join([sen["tokens"][i]["token"] for i in range(len(sen["tokens"]))]))
          print("ppl:",sen["ppl"])
